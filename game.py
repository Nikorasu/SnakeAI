...
class Snake:
    def __init__(self, size=SIZE):
        ...
        self.dir = np.array(rnd.choice([(-1,0),(1,0),(0,-1),(0,1)]), int) # random direction
        self.segments = np.array([self.pos], int) # array of previous positions
        self.len = 3 # starting length of snake
        self.food = np.array([rnd.randint(0,self.rows-1),rnd.randint(0,self.cols-1)], int) # random food position
    ...
    def update(self, newdir=None):
        ...
        if np.all(self.pos == self.food): # if snake eats food
            ...
            field = np.array(np.meshgrid(np.arange(self.rows), np.arange(self.cols))).T.reshape(-1, 2)
...
def main():
    try:
        ...
        player = Snake()
        print(player.state)
        directions = {'w':(-1,0),'s':(1,0),'a':(0,-1),'d':(0,1)} # key directions
